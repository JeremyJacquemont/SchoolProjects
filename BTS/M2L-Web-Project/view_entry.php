...
$link_returl = $returl;  // for use in links
...
<div id="view_entry_nav">
  ...
  if (!$room_disabled)
  {
    ...
    if (!$series)
    {
      $url = formatURLReturnUrl($id, $link_returl, NULL, NULL, NULL, FALSE);
      echo "<a href=\"edit_entry/$url\">". get_vocab("editentry") ."</a>";
    } 
    ...
    if ((!empty($repeat_id) || $series) && $repeats_allowed)
    {
        $url = formatURLReturnUrl($id, $link_returl, $day, $month, $year, TRUE);
      echo "<a href=\"edit_entry/$url\">".get_vocab("editseries")."</a>";
    }
    echo "</div>\n";
    ...
    if (!$series)
    {
      echo "<a href=\"del_entry/id:$id-series:0-returl:$link_returl\" onclick=\"return confirm('".get_vocab("confirmdel")."');\">".get_vocab("deleteentry")."</a>";
    }
    if (!empty($repeat_id) && !$series && $repeats_allowed)
    {
      echo " - ";
    ...
    if ((!empty($repeat_id) || $series) && $repeats_allowed)
    {
      ...
      echo "<a href=\"del_entry/$url/id:$id-series:1-returl:$link_returl\" onClick=\"return confirm('".get_vocab("confirmdel")."');\">".get_vocab("deleteseries")."</a>";
    }
    echo "</div>\n";
  }
  ...
  if (!$series)
  {
    echo "<a href=\"edit_entry/id:$id-copy:1-returl:$link_returl\">". get_vocab("copyentry") ."</a>";
  }      
  if (!empty($repeat_id) && !$series && $repeats_allowed)
  {
    echo " - ";
  ...
  {
    $url = formatURLReturnUrl($id, $link_returl, $day, $month, $year, TRUE);
    echo "<a href=\"edit_entry/$url\">".get_vocab("copyseries")."</a>";
  }
  echo "</div>\n";
