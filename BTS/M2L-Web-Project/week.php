...
else
{
  ...
  for ($j = 0; $j<=($num_of_days-1) ; $j++)
  {
    ...
    $url = formatURLFull(strftime("%Y", $t), strftime("%m", $t), strftime("%d", $t), $area, NULL);
    ...
    {  
      $header .= "<th><a href=\"day/$url\" title=\"" . get_vocab("viewday") . "\">" .
...
$hilite_url="week/$url-timetohighlight";
$row_class = "even_row";
...
if ($times_along_top)
{
  ...
  for ($thisday = 0; $thisday<=($num_of_days-1) ; $thisday++, $row_class = ($row_class == "even_row")?"odd_row":"even_row")
  {
    ...
    else
    {
      ...
      $url = formatURLFull(strftime("%Y", $wt), strftime("%m", $wt), strftime("%d", $wt), $area, NULL);
      $day_cell_text = utf8_strftime($dformat, $wt);
      $day_cell_link = "day/$url";
