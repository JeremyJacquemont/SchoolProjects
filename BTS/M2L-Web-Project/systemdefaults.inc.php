...
$dbsys = getParameter('db', 'db_sys');//"mysql";
// Hostname of database server. For pgsql, can use "" instead of localhost
// to use Unix Domain Sockets instead of TCP/IP.
$db_host = getParameter('db', 'db_host'); //"localhost";
// Database name:
$db_database = getParameter('db', 'db_database');//"mrbs";
// Database login user name:
$db_login = getParameter('db', 'db_login');//"mrbs";
// Database login password:
$db_password = getParameter('db', 'db_password');//'mrbs-password';
// Prefix for table names.  This will allow multiple installations where only
// one database is available
$db_tbl_prefix = getParameter('db', 'db_tbl_prefix');//"mrbs_";
...
$maxlength['entry.name']       = getParameter('db_maxlength', 'entry.name');//80;  // characters   (name field in entry table)
$maxlength['area.area_name']   = getParameter('db_maxlength', 'area.area_name');//30;  // characters   (area_name field in area table)
$maxlength['room.room_name']   = getParameter('db_maxlength', 'room.room_name');//25;  // characters   (room_name field in room table)
$maxlength['room.description'] = getParameter('db_maxlength', 'room.description');//60;  // characters   (description field in room table)
$maxlength['users.name']       = getParameter('db_maxlength', 'users.name');//30;  // characters   (name field in users table)
$maxlength['users.email']      = getParameter('db_maxlength', 'users.email');//75;  // characters   (email field in users table)
 ...
 *********************************/
$mrbs_admin = getParameter('administration', 'mrbs_admin');//"Your Administrator";
$mrbs_admin_email = getParameter('administration', 'mrbs_admin_email');//"admin_email@your.org";
...
$mrbs_company = getParameter('administration', 'mrbs_company');//"M2L Intranet";    
...
// notification.
$url_base = getParameter('administration', 'url_base');//"";
...
$theme = getParameter('theme', 'theme');//"default";
...
$enable_periods = getParameter('calendar', 'enable_periods');//FALSE;  // Default value for new areas
...
$force_resolution = getParameter('calendar', 'force_resolution');//FALSE;

// Default duration - default length (in seconds) of a booking.
// Defaults to (60 * 60) seconds, i.e. an hour
$default_duration = getParameter('calendar', 'default_duration');//(60 * 60);  // DEFAULT VALUE FOR NEW AREAS
...
$morningstarts         = getParameter('calendar', 'morningstarts');//7;   // must be integer in range 0-23
$morningstarts_minutes = getParameter('calendar', 'morningstarts_minutes');//0;   // must be integer in range 0-59

// The beginning of the last slot of the day (DEFAULT VALUES FOR NEW AREAS)
$eveningends           = getParameter('calendar', 'eveningends');//18;  // must be integer in range 0-23
$eveningends_minutes   = getParameter('calendar', 'eveningends_minutes');//30;   // must be integer in range 0-59
...
$max_slots = getParameter('calendar', 'max_slots');//60;
...
$min_book_ahead_enabled = getParameter('booking_policies', 'min_book_ahead_enabled');//FALSE;    // set to TRUE to enforce a minimum advance booking time
$max_book_ahead_enabled = getParameter('booking_policies', 'max_book_ahead_enabled');//FALSE;    // set to TRUE to enforce a maximum advance booking time
...
// DEFAULT VALUES FOR NEW AREAS
$min_book_ahead_secs = getParameter('booking_policies', 'min_book_ahead_secs');//0;           // (seconds) cannot book in the past
$max_book_ahead_secs = getParameter('booking_policies', 'max_book_ahead_secs');//60*60*24*7;  // (seconds) no more than one week ahead
...
$weekstarts = getParameter('display', 'weekstarts');//0;
...
$hidden_days = getParameter('display', 'hidden_days');//array();

// Trailer date format: 0 to show dates as "Jul 10", 1 for "10 Jul"
$dateformat = getParameter('display', 'dateformat');//0;

// Time format in pages. 0 to show dates in 12 hour format, 1 to show them
// in 24 hour format
$twentyfourhour_format = getParameter('display', 'twentyfourhour_format');//1;
...
$display_timezone = getParameter('display', 'display_timezone');//FALSE;

// Results per page for searching:
$search["count"] = getParameter('display', 'search["count"]');//20;

// Page refresh time (in seconds). Set to 0 to disable
$refresh_rate = getParameter('display', 'refresh_rate');//0;
...
$simple_trailer = getParameter('display', 'simple_trailer');//FALSE;

// should areas be shown as a list or a drop-down select box?
$area_list_format = getParameter('display', 'twentyfourhour_format');//"list";
...
// of 12.
$monthly_view_entries_details = getParameter('display', 'monthly_view_entries_details');//"both";

// To view weeks in the bottom (trailer.inc) as week numbers (42) instead of
// 'first day of the week' (13 Oct), set this to TRUE
$view_week_number = getParameter('display', 'view_week_number');//FALSE;
...
$mincals_week_numbers = getParameter('display', 'mincals_week_numbers');//FALSE;
...
$times_along_top = getParameter('display', 'times_along_top');//FALSE;
...
$row_labels_both_sides = getParameter('display', 'row_labels_both_sides');//FALSE;
...
$column_labels_both_ends = getParameter('display', 'column_labels_both_ends');//FALSE;

// Define default starting view (month, week or day)
// Default is day
$default_view = getParameter('display', 'default_view');//"day";
...
// Default is 0
$default_room = getParameter('display', 'default_room');//0;
...
$clipped = getParameter('display', 'clipped');//TRUE;                
...
$month_cell_scrolling = getParameter('display', 'month_cell_scrolling');//TRUE;
...
$max_content_length = getParameter('display', 'max_content_length');//20;  // characters
...
$text_input_max = getParameter('display', 'text_input_max');//70;  // characters
...
$max_rep_entrys = getParameter('miscellaneous', 'max_rep_entrys');//365 + 1;

// Default report span in days:
$default_report_days = getParameter('miscellaneous', 'default_report_days');//60;
...
$javascript_cursor = getParameter('miscellaneous', 'javascript_cursor');//TRUE; // Change to FALSE if clients have very old browsers
                           // incompatible with JavaScript.
$show_plus_link = getParameter('miscellaneous', 'show_plus_link');//FALSE;   // Change to TRUE to always show the (+) link as in
                           // MRBS 1.1.
$highlight_method = getParameter('miscellaneous', 'highlight_method');//"hybrid"; // One of "bgcolor", "class", "hybrid".   "hybrid" is recommended as it is
...
$private_enabled = getParameter('miscellaneous', 'private_enabled');//FALSE;  // DEFAULT VALUE FOR NEW AREAS
           // Display checkbox in entry page to make
           // the booking private.

$private_mandatory = getParameter('miscellaneous', 'private_mandatory');//FALSE;  // DEFAULT VALUE FOR NEW AREAS
...
$private_default = getParameter('miscellaneous', 'private_default');//FALSE;  // DEFAULT VALUE FOR NEW AREAS
...
$private_override = getParameter('miscellaneous', 'private_override');//"none";  // DEFAULT VALUE FOR NEW AREAS
...
$is_private_field['entry.name'] = getParameter('miscellaneous_is_private_field', 'entry.name');//TRUE;
$is_private_field['entry.description'] = getParameter('miscellaneous_is_private_field', 'entry.description');//TRUE;
$is_private_field['entry.create_by'] = getParameter('miscellaneous_is_private_field', 'entry.create_by');//TRUE;
...
$approval_enabled = getParameter('miscellaneous', 'approval_enabled');//FALSE;  // Set to TRUE to enable booking approval

// Set to FALSE if you don't want users to be able to send reminders
// to admins when bookings are still awaiting approval.
$reminders_enabled = getParameter('miscellaneous', 'reminders_enabled');//TRUE;
...
$reminder_interval = getParameter('miscellaneous', 'reminder_interval');//60*60*24*2;  // 2 working days

// Days of the week that are working days (Sunday = 0, etc.)
$working_days = getParameter('miscellaneous', 'working_days');//array(1,2,3,4,5);  // Mon-Fri
...
$confirmation_enabled = getParameter('miscellaneous', 'confirmation_enabled');//TRUE;
...
$confirmed_default = getParameter('miscellaneous', 'confirmed_default');//TRUE;
...
$is_mandatory_field = getParameter('form_values', 'is_mandatory_field');//array();
...
$auth["session"] = getParameter('auth', 'session');//"php"; // How to get and keep the user ID. One of
...
// $auth["type"] = "config"; // How to validate the user/password. One of "none"
                          ...
                          // "nw" "ext".
$auth["type"] = getParameter('auth', 'type');//"db";
...
$auth["session_cookie"]["secret"] = getParameter('auth_session_cookie', 'secret');//"This isn't a very good secret!";
// The expiry time of a session, in seconds. Set to 0 to use session cookies
$auth["session_cookie"]["session_expire_time"] = getParameter('auth_session_cookie', 'session_expire_time');//(60*60*24*30); // 30 days
...
// machines
$auth["session_cookie"]["include_ip"] = getParameter('auth_session_cookie', 'include_ip');//TRUE;
...
// set this to 0.
$auth["session_php"]["session_expire_time"] = getParameter('auth_session_php', 'session_expire_time');//(60*60*24*30); // 30 days
...
$auth["admin"][] = getParameter('auth_admin', 'ip');//"127.0.0.1";     // localhost IP address. Useful with IP sessions.
$auth["admin"][] = getParameter('auth_admin', 'name');//"administrator"; // A user name from the user list. Useful 
...
$auth["user"]["administrator"] = getParameter('auth_user', 'administrator');//"secret";
$auth["user"]["alice"] = getParameter('auth_user', 'alice');//"a";
$auth["user"]["bob"] = getParameter('auth_user', 'bob');//"b";

// 'session_http' configuration settings
$auth["realm"]  = getParameter('auth', 'realm');//"mrbs";
...
$auth["prog"]   = getParameter('auth', 'prog');//"";
$auth["params"] = getParameter('auth', 'params');//"";
...
$max_level = getParameter('auth_params', 'max_level');//2;
// The lowest level of admin allowed to edit other users
$min_user_editing_level = getParameter('auth_params', 'min_user_editing_level');//2;
...
$auth['db_ext']['db_system'] = getParameter('auth_db_ext', 'db_system');//'mysql';
$auth['db_ext']['db_host'] = getParameter('auth_db_ext', 'db_host');//'localhost';
$auth['db_ext']['db_username'] = getParameter('auth_db_ext', 'db_username');//'authuser';
$auth['db_ext']['db_password'] = getParameter('auth_db_ext', 'db_password');//'authpass';
$auth['db_ext']['db_name'] = getParameter('auth_db_ext', 'db_name');//'authdb';
$auth['db_ext']['db_table'] = getParameter('auth_db_ext', 'db_table');//'users';
$auth['db_ext']['column_name_username'] = getParameter('auth_db_ext', 'column_name_username');//'name';
$auth['db_ext']['column_name_password'] = getParameter('auth_db_ext', 'column_name_password');//'password';
// Either 'md5', 'sha1', 'crypt' or 'plaintext'
$auth['db_ext']['password_format'] = getParameter('auth_db_ext', 'password_format');//'md5';
...
$ldap_v3 = getParameter('auth_params', 'ldap_v3');//true;
// If you want to use TLS, change the following to true
$ldap_tls = getParameter('auth_params', 'ldap_tls');//false;
...
$ldap_get_user_email = getParameter('auth_params', 'ldap_get_user_email');//FALSE;
// The LDAP attribute which holds a user's email address
$ldap_email_attrib = getParameter('auth_params', 'ldap_email_attrib');//'mail';
...
// LDAP servers
$ldap_unbind_between_attempts = getParameter('auth_params', 'ldap_unbind_between_attempts');//FALSE;

// Output debugging information for LDAP actions
$ldap_debug = getParameter('auth_params', 'ldap_debug');//FALSE;
...
$imap_host = getParameter('auth_params', 'imap_host');//"imap-server-name";
// The IMAP server port
$imap_port = getParameter('auth_params', 'imap_port');//"143";

// 'auth_imap_php' configuration settings
$auth["imap_php"]["hostname"] = getParameter('auth_imap_php', 'hostname');//"localhost";
...
$pop3_host = getParameter('auth_params', 'pop3_host');//"pop3-server-name";
// The POP3 server port
$pop3_port = getParameter('auth_params', 'pop3_port');//"110";

// 'auth_smtp' configuration settings
$auth['smtp']['server'] = getParameter('auth_smtp', 'server');//'myserver.example.org';

// General settings
// If you want only administrators to be able to book slots, set this
// variable to TRUE
$auth['only_admin_can_book'] = getParameter('auth', 'only_admin_can_book');//FALSE;
// If you want only administrators to be able to make repeat bookings,
// set this variable to TRUE
$auth['only_admin_can_book_repeat'] = getParameter('auth', 'only_admin_can_book_repeat');//FALSE;
// If you want only administrators to be able to make bookings spanning
// more than one day, set this variable to TRUE.
$auth['only_admin_can_book_multiday'] = getParameter('auth', 'only_admin_can_book_multiday');//FALSE;
...
$auth['only_admin_can_select_multiroom'] = getParameter('auth', 'only_admin_can_select_multiroom');//FALSE;
...
$auth['only_admin_can_see_other_users'] = getParameter('auth', 'only_admin_can_see_other_users');//FALSE;
...
$auth['deny_public_access'] = getParameter('auth', 'deny_public_access');//TRUE;
...
$mail_settings['admin_on_bookings']      = getParameter('mail', 'admin_on_bookings');//FALSE;  // the addresses defined by $mail_settings['recipients'] below
$mail_settings['area_admin_on_bookings'] = getParameter('mail', 'area_admin_on_bookings');//FALSE;  // the area administrator
$mail_settings['room_admin_on_bookings'] = getParameter('mail', 'room_admin_on_bookings');//FALSE;  // the room administrator
$mail_settings['booker']                 = getParameter('mail', 'booker');//FALSE;  // the person making the booking
$mail_settings['book_admin_on_approval'] = getParameter('mail', 'book_admin_on_approval');//FALSE;  // the booking administrator when booking approval is enabled
...
$mail_settings['on_new']    = getParameter('mail', 'on_new');//TRUE;   // when an entry is created
$mail_settings['on_change'] = getParameter('mail', 'on_change');//FALSE;  // when an entry is changed
$mail_settings['on_delete'] = getParameter('mail', 'on_delete');//FALSE;  // when an entry is deleted
...
// Set to TRUE or FALSE as required
$mail_settings['details']   = getParameter('mail', 'details');//FALSE; // Set to TRUE if you want full booking details;
                                     // otherwise you just get a link to the entry
$mail_settings['html']      = getParameter('mail', 'html');//FALSE; // Set to true if you want HTML mail
$mail_settings['icalendar'] = getParameter('mail', 'icalendar');//FALSE; // Set to TRUE to include iCalendar details
...
$mail_settings['admin_lang'] = getParameter('mail', 'admin_lang');//'en';   // Default is 'en'.
...
$mail_settings['domain'] = getParameter('mail', 'domain');//'';
...
$mail_settings['username_suffix'] = getParameter('mail', 'username_suffix');//'';
...
$mail_settings['admin_backend'] = getParameter('mail', 'admin_backend');//'mail';
...
$sendmail_settings['path'] = getParameter('sendmail', 'path');//'/usr/bin/sendmail';
// Set additional Sendmail parameters (only used with "sendmail" backend).
// (example "-t -i"). Default is ''
$sendmail_settings['args'] = getParameter('sendmail', 'args');//'';
...
$smtp_settings['host'] = getParameter('smtp', 'host');//'localhost';  // SMTP server
$smtp_settings['port'] = getParameter('smtp', 'port');//25;           // SMTP port number
$smtp_settings['auth'] = getParameter('smtp', 'auth');//FALSE;        // Whether to use SMTP authentication
$smtp_settings['username'] = getParameter('smtp', 'username');//'';       // Username (if using authentication)
$smtp_settings['password'] = getParameter('smtp', 'password');//'';       // Password (if using authentication)
...
$mail_settings['from'] = getParameter('mail', 'from');//'admin_email@your.org';

// Set the recipient email. Default is 'admin_email@your.org'. You can define
// more than one recipient like this "john@doe.com,scott@tiger.com"
$mail_settings['recipients'] = getParameter('mail', 'recipients');//'admin_email@your.org';

// Set email address of the Carbon Copy field. Default is ''. You can define
// more than one recipient (see 'recipients')
$mail_settings['cc'] = getParameter('mail', 'cc');//'';
...
// fields are set)
$mail_settings['treat_cc_as_to'] = getParameter('mail', 'treat_cc_as_to');//FALSE;

// The filename to be used for iCalendar attachments.   Will always have the
// extension '.ics'
$mail_settings['ics_filename'] = getParameter('mail', 'ics_filename');//"booking";
...
// as specified below
$disable_automatic_language_changing = getParameter('language', 'disable_automatic_language_changing');//0;
...
$default_language_tokens = getParameter('language', 'default_language_tokens');//"en";
...
// use "_fr"
$faqfilelang = getParameter('language', 'faqfilelang');//""; 
...
$report_filename  = getParameter('reports', 'report_filename');//"report.csv";
$summary_filename = getParameter('reports', 'summary_filename');//"summary.csv";

// CSV format
$csv_row_sep = getParameter('reports', 'csv_row_sep');//"\n";  // Separator between rows/records
$csv_col_sep = getParameter('reports', 'csv_col_sep');//",";   // Separator between columns/fields
...
// $typel["C"] = "C";
// $typel["D"] = "D";
$typel["E"] = getParameter('entry_types', 'typel["E"]');//"E";
// $typel["F"] = "F";
// $typel["G"] = "G";
// $typel["H"] = "H";
$typel["I"] = getParameter('entry_types', 'typel["I"]');//"I";
// $typel["J"] = "J";

// Default type for new bookings
$default_type = getParameter('entry_types', 'default_type');//"I";

// Default description for new bookings
$default_description = getParameter('entry_types', 'default_description');//"";
