...
echo make_area_select_html('day', $area, $year, $month, $day);
...
<div class=\"screenonly\">
  ...
  else
  {
    ...
    $url = formatURLFull($year, $month, $day, $area, $row['id']);
  ...
  $hilite_url="day/$url-timetohighlight";
  ...
  if ($times_along_top)
  {
    ...
    for ($i = 0; ($row = sql_row_keyed($res, $i)); $i++, $row_class = ($row_class == "even_row")?"odd_row":"even_row")
    {
      echo "<tr>\n";
      $room_id = $row['id']; 
      $url = formatURLFull($year, $month, $day, $area, $room_id);
      $room_cell_link = "week/$url";
